...
import { useRouter } from 'next/navigation'; // Corrected import and usage location
...
export default function PTSDPage() {
  ...
  // ** Effects **
  useEffect(() => {
    // Animation for the infinite logo scroll
    controls.start({
      x: "-50%",
      transition: {
        duration: 40,
  ...
  return (
    <main className="w-full bg-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-16 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl font-black text-slate-950 mb-6 leading-[1.1] tracking-tight">
              Reclaim Safety. <span className="text-[#306EFF]">Process</span>{" "}
              History.
            </h1>

            <p className="text-xl md:text-2xl text-slate-800 font-medium leading-relaxed mb-8 max-w-xl">
              Trauma can trap the mind in the past. We provide a clinically safe
              space to stabilize triggers and rebuild your foundation of peace.
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div>
            <h2 className="text-3xl md:text-4xl font-black text-slate-950 mb-8 leading-tight">
              A Clinically <span className="text-[#306EFF]">Stabilizing</span>{" "}
              Environment.
            </h2>
            <p className="text-lg text-slate-700 leading-relaxed mb-8">
              Post-Traumatic Stress Disorder requires more than just conversation;
              it requires neurological regulation. We combine medication
              management for hypervigilance with trauma-informed therapy to
              create lasting recovery.
            </p>
          ...
          <div className="bg-slate-900 p-10 md:p-14 rounded-[40px] text-white shadow-2xl relative">
            ...
            <p className="text-slate-400 text-lg leading-relaxed mb-8 font-medium">
              Our intake process is patient-led. You set the pace. We provide the
              expertise to manage symptoms like flash-backs and sleep disruption.
            </p>
            <Link
              href="/appointments"
              className="inline-flex items-center gap-2 font-black text-[#306EFF] hover:text-white transition group"
            >
      ...
      <section className="relative py-24 bg-[#F8F9FA] border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-16 flex flex-col lg:flex-row justify-between items-center mb-16 gap-8">
          ...
          <div className="max-w-xl text-center lg:text-left">
            ...
            <p className="text-slate-600 text-lg font-medium leading-relaxed">
              We focus on clinical safety; let insurance focus on the cost. We
              accept a wide range of providers to ensure PTSD care remains
              accessible.
            </p>
          </div>
          {/* Badges */}
          <div className="flex gap-4">
            <div className="px-6 py-3 bg-white rounded-2xl border border-slate-200 text-slate-900 font-bold shadow-sm">
              Verified Providers
            </div>
            <div className="px-6 py-3 bg-white rounded-2xl border border-slate-200 text-slate-900 font-bold shadow-sm">
              Secure Billing
        ...
        {/* Desktop - Infinite Scroll Logo Carousel */}
        <div className="hidden lg:flex relative w-full overflow-hidden group">
          ...
          >
            {[...insuranceLogos, ...insuranceLogos].map((logo, i) => (
              <div
                key={i}
                className="flex items-center justify-center w-[240px] h-[140px] bg-white rounded-3xl border border-slate-200 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl hover:border-[#306EFF]/20 transition-all duration-300"
              >
        ...
        {/* Mobile - Expandable Logo Grid */}
        <div className="lg:hidden px-6 max-w-md mx-auto">
          <div className="grid grid-cols-2 gap-4">
            {(isExpanded ? insuranceLogos : insuranceLogos.slice(0, 6)).map(
              (logo, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl shadow-sm p-4 h-28 flex items-center justify-center border border-slate-100"
                >
                  <Image
                    src={logo.src}
                    alt={logo.name}
          ...
          <button
            ...
            className="mt-8 mx-auto flex items-center gap-2 font-black text-slate-900 bg-white border border-slate-200 px-8 py-3 rounded-2xl hover:bg-slate-50 transition-colors shadow-sm"
          >
            {isExpanded ? (
              <>
                Collapse List <FaChevronUp />
              </>
            ) : (
              <>
                See Full Network ({insuranceLogos.length}) <FaChevronDown />
              </>
            )}
          </button>
        </div>
      ...
      <section className="py-24 text-center">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <button
            ...
            className="bg-slate-950 text-white font-black px-12 py-5 rounded-2xl text-xl hover:bg-[#306EFF] transition-all flex items-center gap-3 mx-auto shadow-2xl"
          >
